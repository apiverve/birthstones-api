...
class BirthstonesValidationException implements Exception {
  final List<String> errors;

  BirthstonesValidationException(this.errors);

  @override
  String toString() => 'BirthstonesValidationException: ${errors.join("; ")}';
}
...
/// Birthstones API client.
///
/// For more information, visit: https://apiverve.com/marketplace/birthstones?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [month] (required) - The month number (1-12) or name
class BirthstonesClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'month': ValidationRule(type: 'string', required: true),
  };

  BirthstonesClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/birthstones',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BirthstonesValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BirthstonesValidationException(errors);
    }
  }

  /// Execute a request to the Birthstones API.
  ///
  /// Parameters are validated before sending the request.
  Future<BirthstonesResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BirthstonesException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BirthstonesResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BirthstonesException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BirthstonesException('Resource not found');
      } else {
        throw BirthstonesException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BirthstonesException) rethrow;
      throw BirthstonesException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Birthstones API client.
class BirthstonesException implements Exception {
  final String message;

  BirthstonesException(this.message);

  @override
  String toString() => 'BirthstonesException: $message';
}
